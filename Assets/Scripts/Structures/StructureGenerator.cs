...
public class StructureGenerator
{
    ...
    {
        StructureConfigData config = ConfigManager.Instance?.StructConfig;

        if (config?.structures == null)
        {
            Debug.LogWarning("[StructureGenerator] StructureConfig not loaded — using no structures");
            return;
        }

        foreach (StructureEntry entry in config.structures)
        {
            Structure structure = StructureFactory.Create(entry);
            if (structure == null) continue;

            foreach (int biomeID in entry.allowedBiomes)
                RegisterStructure(biomeID, structure, entry.spawnChance);
        }

        Debug.Log($"[StructureGenerator] Registered structures for {_biomeStructureRules.Count} biomes");
    }
    ...
    private bool IsTooClose(HashSet<Vector2Int> used, int x, int z)
    {
        for (int dx = -MinSpacing; dx <= MinSpacing; dx++)
            for (int dz = -MinSpacing; dz <= MinSpacing; dz++)
                if (used.Contains(new Vector2Int(x + dx, z + dz))) return true;
        return false;
    }
    ...
    private int FindSurfaceY(int[,,] voxelMap, int x, int z, int chunkWidth, int chunkHeight)
    {
        for (int y = chunkHeight - 1; y >= 0; y--)
            if (voxelMap[x, y, z] != BlockTypes.AirID) return y;
        return -1;
    }
}